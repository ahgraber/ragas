...
def test_executor_jupyter_ipython(monkeypatch):
    """Test that RuntimeError is raised in Jupyter/IPython environment."""
    executor = Executor()

    class DummyIPython:
        pass

    monkeypatch.setattr("IPython.core.getipython.get_ipython", lambda: DummyIPython())

    async def echo():
        return 1

    executor.submit(echo)
